...
struct Searcher {
    ...
    bool combinable(Shape shape,
        ...
        for (size_t angle = 0; angle < PART / 2; ++angle) {
            ...
            left = left.equivalentHalve();
            right = right.equivalentHalve();
    ...
    void run() {
        ...
        quadSearcher.run();
        std::cout << "Found " << quadSearcher.quads.size() << " quarters" << std::endl;
        ...
        if constexpr (PART == 4) {
            ...
            for (size_t i = 0; i < total; ++i) {
                ...
                half = half.collapse();
                half = half.equivalentHalve();
                if (halvesIdx.emplace(half, halves.size()).second) {
                    halves.push_back(half);
                }
            }
            std::cout << "Pre-calculated " << halves.size() << " halves" << std::endl;
        } else {
            ...
            if (nextHalf < halves.size()) {
                ...
                for (size_t i = 0; i <= nextHalf; ++i) {
                    ...
                    for (Shape a : variants) {
                        ...
                        Shape shape = combined.equivalentShape();
    ...
    void process(Shape shape) {
        ...
        if (count >= nextLogCount) {
            ...
            std::cout << "Processed " << count << " shapes, "
                    << quarters.size() << " quarters, "
                    << nextHalf << "/" <<  halves.size()<< " halves, "
                    << queueSet.size() << "/" << queue.size() << "/" << shapes.size() << " shapes"
                    << std::endl;
        }
        ...
        // cut
        for (size_t angle = 0; angle < PART; ++angle) {
            Shape cut = shape.rotate(angle).cut().equivalentHalve();
